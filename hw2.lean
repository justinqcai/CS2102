...
Answer here: 
Yes, G is total because its domain is its entire domain of definition. {1,2,3}, its domain, is the same as {1,2,3}, its domain of definition.
-/
...
Answer here: 
4 is a value in the domain of definition that isn't defined in the domain of F, meaning there is at least one value in its
domain of definition where there is no corresponding value in the codomain, proving F is strictly partial.
-/
...
Answer here: 
11 is a value in the codomain of F that proves that F is not surjective because its not in the range of F.
-/
...
Answer here: 
G is not strictly partial because its defined domain is its entire domain of definition, meaning every value in the domain
of definition has a corresponding image y in the codomain.
-/
...
Answer here: 
40 is a value in the codomain of F that is not the image of any value in its domain of definition.
-/
...
Answer here: 
A function is considered injective if each value in its domain has exactly one unique image, meaning no two domain values
can have the same value in its range.
-/
...
Answer here: 
log(x) is injective because for each value in its domain (all real numbers), there is a unique value in its range.
It's surjective because its range is its entire codomain (all real numbers). It's bijective because no two domain
values have the same range value.
-/
...
Answer here: 
No, the relation is not single-valued because the same x can be paired with at least two different y's (positive and
negative) and still satisfy the relation.
-/
